...
**提醒时效**：突破提醒讲究时效，`AlertTTL = 30min`（`internal/futures/watch.go`）—— K 线时间早于「现在 − 30min」的事件**不进列表、不外发**，服务端在每轮扫描结束和每次读取 `events` 时都会清理，前端每轮轮询同步剔除，所以过期的提醒会**自动从列表消失**。状态里下发 `alert_ttl_sec`，前端文案按它渲染；被清理的事件仍留在去重集合里，同一根 K 线不会被重复提醒。
