...
func TestRecommendPrices(t *testing.T) {
	...
	// 做空镜像
	stop, tp = RecommendPrices("RB", DirDown, 100, 4, 1, 1.5)
	if stop != 104 || tp != 94 {
		t.Fatalf("做空推荐不对：stop=%v tp=%v", stop, tp)
	}

	// 盈亏比没给 → 用默认 1.5
	_, tp = RecommendPrices("RB", DirUp, 100, 4, 1, 0)
	if tp != 106 {
		t.Fatalf("默认盈亏比应为 %v：tp=%v", DefaultRR, tp)
	}

	// 自定义盈亏比 = 3
	_, tp = RecommendPrices("RB", DirUp, 100, 4, 1, 3)
	if tp != 112 {
	...
	for _, atr := range []float64{0, -1, math.NaN()} {
		if s, p := RecommendPrices("RB", DirUp, 100, atr, 1, 1.5); s != 0 || p != 0 {
			t.Fatalf("atr=%v 应返回 0：stop=%v tp=%v", atr, s, p)
		}
	}
	if s, p := RecommendPrices("RB", DirUp, 0, 4, 1, 1.5); s != 0 || p != 0 {
		t.Fatalf("没价格应返回 0：stop=%v tp=%v", s, p)
	}

	// 负盈亏比按默认处理，不倒挂
	stop, tp = RecommendPrices("RB", DirUp, 100, 4, 1, -2)
	if stop != 96 || tp != 106 {
